...
if($submit == 'status'){
	...
	if($newstatus == '5'){
		$SQL = "DELETE FROM todo_raf WHERE id='".$todo_id."'";
		# SQL ausführen
		mysql_query($SQL);
	...
	$SQL = "UPDATE todo_raf SET status='".$newstatus."' WHERE id='".$todo_id."'";
	# SQL ausführen
	mysql_query($SQL);
	#Anzahl betroffener Datensätze ausgeben:
...
elseif($submit == 'hinzu') {
	...
	$SQL = "INSERT INTO todo_raf (inhalt, status) VALUES ('".$inhalt."', '".$status."')";
...
// ToDo Ausgabe *********************************************************************************	
$query = mysql_query("SELECT id, inhalt, status, DATE_FORMAT(datum, GET_FORMAT(DATE,'EUR')) as 'datumform' FROM todo_raf") or die(mysql_error());
