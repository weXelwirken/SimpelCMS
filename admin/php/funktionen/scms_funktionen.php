...
function OrdPfadBaumOpt($mutter, $mutterAlt, $mutterAkt, $mutterName, $ebene, $id)
	...
	if($ergebnis)
		...
		while($zeile = mysql_fetch_array($ergebnis))
			...
			if($ScmsLang){$langOut=" [".$zeile["Lang"]."]";}
			echo "<option value=\"".$zeile["ID"]."\"".$selected.">".$ebene.$zeile["Name"].$langOut." [".$zeile["Position"]."]</option>\n";
			OrdPfadBaumOpt($zeile["ID"], $mutterAlt, $mutterAkt, $zeile["Name"], $ebene, $id);}
			}
		}
...
function OrdPosBesOpt()
	...
	// OrdnerDaten aus DB holen
	// SQL-String bauen
	echo $sqlString = "SELECT * FROM "._DBordner." ORDER BY Position";
	...
	if($ergebnis)
		...
		while($zeile = mysql_fetch_array($ergebnis))
			{
			echo "<option>".$zeile["Position"].": &raquo;".$zeile["Name"]."&laquo;</option>\n";
			}
		}
